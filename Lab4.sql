...
*/
select p.cus_gender as Gender ,count(cus_gender) as NoOfCustomers from(select c.cus_id,cus_gender from customer c 
join `order` o on c.cus_id=o.cus_id where ord_amount>=3000 group by c.cus_id) p 
...
*/
select o.ord_id,o.ord_amount,o.ord_date,p.pro_name from `order`as o 
...
*/
select s.* from supplier s where s.supp_id in (select supp_id from supplier_pricing group by supp_id having count(supp_id)>1);
